...
namespace Plugins
{
    ...
    public class RegistrationPreferenceCreate : PluginBase
    {
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localContext)
        {
            ...
            if (context.InputParameters.Contains("Target"))
            {
                if (context.InputParameters["Target"] is Entity)
                {
                    ...
                    {
                        queriedEntityRecord = service.Retrieve("msnfp_registrationpreference", targetIncomingRecord.Id, GetColumnSet());
                    }
                ...
                {
                    queriedEntityRecord = service.Retrieve("msnfp_registrationpreference", ((EntityReference)context.InputParameters["Target"]).Id, GetColumnSet());
        ...
        private void AddOrUpdateThisRecordWithAzure(Entity queriedEntityRecord, Entity configurationRecord, LocalPluginContext localContext, IOrganizationService service, IPluginExecutionContext context)
        {
            ...
            string messageName = context.MessageName;
            string entityName = "RegistrationPreference"; // Used for API calls
            ...
            if (apiUrl != string.Empty)
            {
                ...
                MSNFP_RegistrationPreference jsonDataObj = new MSNFP_RegistrationPreference();

                jsonDataObj.registrationpreferenceid = (Guid)queriedEntityRecord["msnfp_registrationpreferenceid"];

                if (queriedEntityRecord.Contains("msnfp_registrationid") && queriedEntityRecord["msnfp_registrationid"] != null)
                {
                    jsonDataObj.registrationid = ((EntityReference)queriedEntityRecord["msnfp_registrationid"]).Id;
                    localContext.TracingService.Trace("Got msnfp_registrationid");
                }
                else
                {
                    jsonDataObj.registrationid = null;
                ...
                {
                    jsonDataObj.eventpreference = ((EntityReference)queriedEntityRecord["msnfp_eventpreference"]).Id;
                    localContext.TracingService.Trace("Got msnfp_eventpreference");
                }
                else
                {
                    jsonDataObj.eventpreference = null;
                ...
                {
                    jsonDataObj.other = (string)queriedEntityRecord["msnfp_other"];
                    localContext.TracingService.Trace("Got msnfp_other.");
                }
                else
                {
                    jsonDataObj.other = null;
                ...
                DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(MSNFP_RegistrationPreference));
