...
namespace Plugins
{
    ...
    public class MembershipCreate : PluginBase
    {
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localContext)
        {
            ...
            if (context.InputParameters.Contains("Target"))
            {
                if (context.InputParameters["Target"] is Entity)
                {
                    ...
                    {
                        queriedEntityRecord = service.Retrieve("msnfp_membership", targetIncomingRecord.Id, GetColumnSet());
                    }
                    ...
                    if (targetIncomingRecord != null)
                    {
                        ...
                        if (messageName == "Create")
                        {
                            ...
                            UpdatePrimaryMembership(targetIncomingRecord, service, localContext.TracingService);
                        }
                        else if (messageName == "Update")
                        {
                            AddOrUpdateThisRecordWithAzure(queriedEntityRecord, configurationRecord, localContext, service, context);
                            UpdatePrimaryMembership(queriedEntityRecord, service, localContext.TracingService);
                        }
                    }
                    else
                    {
                ...
                {
                    queriedEntityRecord = service.Retrieve("msnfp_membership", ((EntityReference)context.InputParameters["Target"]).Id, GetColumnSet());
        ...
        {            
            var msnfp_customer = target.GetAttributeValue<EntityReference>("msnfp_customer");
            if (msnfp_customer == null || 
                target.GetAttributeValue<bool?>("msnfp_primary") != true) return;

            using (var orgSvcContext = new OrganizationServiceContext(service))
            {
                var _customer = new Entity { LogicalName = msnfp_customer.LogicalName, Id = msnfp_customer.Id };
                _customer["msnfp_primarymembershipid"] = target.ToEntityReference();
                service.Update(_customer);
              
                var ids = from m in orgSvcContext.CreateQuery("msnfp_membership")
                          where (bool)m["msnfp_primary"] == true &&
                           (Guid)m["msnfp_membershipid"] != target.Id &&
                           (EntityReference)m["msnfp_customer"] != null &&
                           ((EntityReference)m["msnfp_customer"]) == msnfp_customer
                          select m.Id;
                            
                foreach(var id in ids)
                {
                    var _m = new Entity("msnfp_membership", id);
                    _m["msnfp_primary"] = false;
                    service.Update(_m);
                }
            }                
        }
        ...
        private void AddOrUpdateThisRecordWithAzure(Entity queriedEntityRecord, Entity configurationRecord, LocalPluginContext localContext, IOrganizationService service, IPluginExecutionContext context)
        {
            ...
            string messageName = context.MessageName;
            string entityName = "Membership"; // Used for API calls
            ...
            if (apiUrl != string.Empty)
            {
                ...
                MSNFP_Membership jsonDataObj = new MSNFP_Membership();

                jsonDataObj.MembershipId = (Guid)queriedEntityRecord["msnfp_membershipid"];
                ...
                {
                    jsonDataObj.Customer = ((EntityReference)queriedEntityRecord["msnfp_customer"]).Id;
                    localContext.TracingService.Trace("Got msnfp_customer.");
                }
                else
                {
                    jsonDataObj.Customer = null;
                ...
                {
                    jsonDataObj.Primary = (bool)queriedEntityRecord["msnfp_primary"];
                    localContext.TracingService.Trace("Got msnfp_primary.");
                }
                else
                {
                    jsonDataObj.Primary = null;
                ...
                jsonDataObj.MembershipCategory = null;

                jsonDataObj.MembershipOrder = new HashSet<MSNFP_MembershipOrder>();
                ...
                DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(MSNFP_Membership));
