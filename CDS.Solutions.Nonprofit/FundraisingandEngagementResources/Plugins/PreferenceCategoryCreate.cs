...
namespace Plugins
{
    ...
    public class PreferenceCategoryCreate : PluginBase
    {
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localContext)
        {
            ...
            if (context.InputParameters.Contains("Target"))
            {
                if (context.InputParameters["Target"] is Entity)
                {
                    ...
                    {
                        queriedEntityRecord = service.Retrieve("msnfp_preferencecategory", targetIncomingRecord.Id, GetColumnSet());
                    }
                ...
                {
                    queriedEntityRecord = service.Retrieve("msnfp_preferencecategory", ((EntityReference)context.InputParameters["Target"]).Id, GetColumnSet());
        ...
        private void AddOrUpdateThisRecordWithAzure(Entity queriedEntityRecord, Entity configurationRecord, LocalPluginContext localContext, IOrganizationService service, IPluginExecutionContext context)
        {
            ...
            string messageName = context.MessageName;
            string entityName = "PreferenceCategory"; // Used for API calls
            ...
            if (apiUrl != string.Empty)
            {
                ...
                MSNFP_PreferenceCategory jsonDataObj = new MSNFP_PreferenceCategory();

                jsonDataObj.preferencecategoryid = (Guid)queriedEntityRecord["msnfp_preferencecategoryid"];

                if (queriedEntityRecord.Contains("msnfp_categorycode") && queriedEntityRecord["msnfp_categorycode"] != null)
                {
                    jsonDataObj.categorycode = ((OptionSetValue)queriedEntityRecord["msnfp_categorycode"]).Value;
                    localContext.TracingService.Trace("Got msnfp_categorycode.");
                }
                else
                {
                    jsonDataObj.categorycode = null;
                ...
                DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(MSNFP_PreferenceCategory));
