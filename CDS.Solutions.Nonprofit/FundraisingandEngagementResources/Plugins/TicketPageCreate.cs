...
namespace Plugins
{
    ...
    public class TicketPageCreate : PluginBase
    {
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localContext)
        {
            ...
            if (context.InputParameters.Contains("Target"))
            {
                if (context.InputParameters["Target"] is Entity)
                {
                    ...
                    {
                        queriedEntityRecord = service.Retrieve("msnfp_ticket", targetIncomingRecord.Id, GetColumnSet());
                    }
                    ...
                    if (targetIncomingRecord != null)
                    {
                        ...
                        {
                            UpdateEventPackageTicketTotals(targetIncomingRecord, orgSvcContext, service, localContext);
                            UpdateEventTicketTotals(targetIncomingRecord, orgSvcContext, service, localContext);
                            AddOrUpdateThisRecordWithAzure(targetIncomingRecord, configurationRecord, localContext, service, context);
                        }
                        else if (messageName == "Update")
                        {
                            UpdateEventPackageTicketTotals(queriedEntityRecord, orgSvcContext, service, localContext);
                            UpdateEventTicketTotals(queriedEntityRecord, orgSvcContext, service, localContext);
                ...
                {
                    queriedEntityRecord = service.Retrieve("msnfp_ticket", ((EntityReference)context.InputParameters["Target"]).Id, GetColumnSet());
        ...
        private void AddOrUpdateThisRecordWithAzure(Entity queriedEntityRecord, Entity configurationRecord, LocalPluginContext localContext, IOrganizationService service, IPluginExecutionContext context)
        {
            ...
            string messageName = context.MessageName;
            string entityName = "Ticket"; // Used for API calls
            ...
            if (apiUrl != string.Empty)
            {
                ...
                MSNFP_Ticket jsonDataObj = new MSNFP_Ticket();

                jsonDataObj.TicketId = (Guid)queriedEntityRecord["msnfp_ticketid"];
                ...
                DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(MSNFP_Ticket));
        ...
        private void UpdateEventPackageTicketTotals(Entity queriedEntityRecord, OrganizationServiceContext orgSvcContext, IOrganizationService service, LocalPluginContext localContext)
        {
            ...
            if (queriedEntityRecord.Contains("msnfp_eventpackageid"))
            {
                ...
                foreach (Entity item in ticketList)
                {
                    if (item.Contains("msnfp_amount") && item["msnfp_amount"] != null)
                        valAmount += ((Money)item["msnfp_amount"]).Value;
                }

                eventPackage["msnfp_sum_tickets"] = ticketList.Count(); // # tickets
                eventPackage["msnfp_val_tickets"] = new Money(valAmount); // $ tickets
        ...
        #region Updating Event Ticket Totals
        ...
        private void UpdateEventTicketTotals(Entity queriedEntityRecord, OrganizationServiceContext orgSvcContext, IOrganizationService service, LocalPluginContext localContext)
        {
            ...
            {
                decimal valAmount = 0;
                Entity eventTicket = service.Retrieve("msnfp_eventticket", ((EntityReference)queriedEntityRecord["msnfp_eventticketid"]).Id, new ColumnSet(new string[] { "msnfp_eventticketid", "msnfp_amount", "msnfp_quantity" }));
                ...
                eventTicket["msnfp_sum_sold"] = ticketList.Count(); // tickets sold
                eventTicket["msnfp_val_sold"] = new Money(valAmount); //$ tickets sold

                if (eventTicket.Contains("msnfp_quantity") && eventTicket["msnfp_quantity"] != null)
                    eventTicket["msnfp_sum_available"] = (int)eventTicket["msnfp_quantity"] - ticketList.Count(); // tickets available
