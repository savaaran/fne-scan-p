...
function onCampaignChange(executionContext) {
    let formContext = executionContext.getFormContext();
    let msnfp_originatingcampaign = formContext.getAttribute("msnfp_originatingcampaignid").getValue();
    if (msnfp_originatingcampaign) {
        Xrm.WebApi.online.retrieveRecord("campaign", msnfp_originatingcampaign[0].id, "?$expand=msnfp_Campaign_DefaultDesignation($select=msnfp_designationid,msnfp_name)").then(
            (result) => {
                ...
                formContext.getAttribute("msnfp_designationid").setValue([{
...
function onSave(executionContext) {
    ...
    if (formType === 1 && (saveMode === 1 || saveMode === 2)) {
        formContext.ui.setFormNotification("Clicking Save or Save and Close will not process a transaction. Selecting Process is the only way to complete a Payment Schedule", "ERROR", "121");
